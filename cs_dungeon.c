...
void player_stats(struct map *map)
{
    ...
    while (current != NULL)
    {
        if (current->contains_player) // Check
        {
                   ...
                   int num_monsters, int position)
{
    ...
    else
    {
        ...
        current = map->entrance;
        int index = 1;
        struct dungeon *prev = NULL; // Handle
...
int fight(struct map *map, char command)
{
    ...
    {
        player_attack = map->player->damage; // noted
    }
    ...
    for (int i = 0; i < current->num_monsters; i++)
    {
        ...
        {
            remaining_attack -= monster_health; // noted
            monsters_defeated++;
        }
        else
        {
...
int end_turn(struct map *map)
{
    ...
    if (player->health_points <= 0) // Handle
    {
    ...
    if (player->points >= points_required && IS_ACTIVE == IS_CONDITION)
    {
        ...
        while (current != NULL)
        {
            if (current->num_monsters > 0)
            {
                all_monsters_defeated = 0; // Handle
            }
...
int collect_item(struct map *map, int item_number)
{
    ...
    {
        if (current_dungeon->contains_player) // Check
        {
            break;
        }
